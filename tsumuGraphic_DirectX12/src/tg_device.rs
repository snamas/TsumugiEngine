...
use crate::tg_directx::{CpD3D12_CPU_DESCRIPTOR_HANDLE, CpID3D12CommandAllocator, CpID3D12DescriptorHeap, CpID3D12Fence, CpID3D12PipelineState, CpID3D12Resource, CpID3D12RootSignature, CpID3DBlob};
use crate::tg_graphics_command_list::CpID3D12GraphicsCommandList;

pub struct CpID3D12Device(pub *const ID3D12Device);
impl CpID3D12Device {
    ...
    pub fn cp_create_descriptor_heap(&self, heap_desc_for_swapchain_opt: Option<D3D12_DESCRIPTOR_HEAP_DESC>) -> Result<CpID3D12DescriptorHeap, HRESULT> {
        ...
        let mut _unknownobj = null_mut();
        unsafe {
            match self.0.as_ref().unwrap().CreateDescriptorHeap(&heap_desc_for_swapchain, &ID3D12DescriptorHeap::uuidof(), &mut _unknownobj).result() {
                Ok(v) => {
    ...
    pub fn cp_create_command_list(&self, node_mask: UINT, type_: D3D12_COMMAND_LIST_TYPE, p_command_allocator: &mut CpID3D12CommandAllocator, p_initial_state_opt: &mut Option<ID3D12PipelineState>) -> Result<CpID3D12GraphicsCommandList, HRESULT> {
        ...
        unsafe {
            let mut _unknownobj = null_mut();
            match self.0.as_ref().unwrap().CreateCommandList(node_mask, type_, p_command_allocator.0, null_mut(), &ID3D12GraphicsCommandList::uuidof(), &mut _unknownobj).result() {
                Ok(v) => {
    ...
    pub fn cp_create_render_target_view<T>(&self, pResource: &mut CpID3D12Resource<T>, pDesc_opt: Option<D3D12_RENDER_TARGET_VIEW_DESC>, DestDescriptor: CpD3D12_CPU_DESCRIPTOR_HANDLE) {
        ...
        unsafe { self.0.as_ref().unwrap().CreateRenderTargetView(pResource.value, pDesc, DestDescriptor.value) }
    }
    ...
    pub fn cp_create_buffer_resource<T>(&self, nodemask: u32, vertices: &Box<[T]>) -> Result<CpID3D12Resource<Box<[T]>>, HRESULT> {
    ...
    pub fn cp_create_index_resource(&self, nodemask: u32, indices: &Box<[u32]>) -> Result<CpID3D12Resource<Box<[u32]>>, HRESULT> {
        let heapProperties = D3D12_HEAP_PROPERTIES {
            Type: D3D12_HEAP_TYPE_UPLOAD,
            CPUPageProperty: D3D12_CPU_PAGE_PROPERTY_UNKNOWN,
            MemoryPoolPreference: D3D12_MEMORY_POOL_UNKNOWN,
