...
unsafe impl Send for CpID3D12CommandQueue {}

unsafe impl Sync for CpID3D12CommandQueue {}
impl CpID3D12CommandQueue {
    ...
    pub fn cp_execute_command_list(&self, cp_id3d12command_lists: &mut CpID3D12GraphicsCommandList) {
        let ppCommandLists = cp_id3d12command_lists.0 as *mut ID3D12CommandList;
        unsafe {
            self.value.as_ref().unwrap().ExecuteCommandLists(1,  &ppCommandLists);
        }
    }
